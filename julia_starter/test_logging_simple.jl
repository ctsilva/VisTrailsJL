...
# Module 1 - successful
mod_exec1 = ModuleExec(
    module_id=1,
    module_name="ReadFile",
    machine_id=machine.id
)
sleep(0.05)  # Simulate work
mark_completed!(mod_exec1)
add_module_exec!(wf_exec, mod_exec1)
println("   ✓ Module 1 (ReadFile) - completed")

# Module 2 - cached
mod_exec2 = ModuleExec(
    module_id=2,
    module_name="ProcessData",
    machine_id=machine.id
)
mark_cached!(mod_exec2)
add_module_exec!(wf_exec, mod_exec2)
println("   ✓ Module 2 (ProcessData) - cached")

# Module 3 - failed
mod_exec3 = ModuleExec(
    module_id=3,
    module_name="WriteFile",
    machine_id=machine.id
)
sleep(0.03)  # Simulate work
mark_failed!(mod_exec3, "Permission denied")
add_module_exec!(wf_exec, mod_exec3)
...
println("  Cached: $(length(cached_modules(wf_exec)))")
println("  Failed: $(length(failed_modules(wf_exec)))")
...
log = Log(vistrail_id="test_vistrail")
add_workflow_exec!(log, wf_exec)
